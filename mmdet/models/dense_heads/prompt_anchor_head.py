...
class StaRPNHead(nn.Module):
    ...
    def __init__(self, hidden_dim, num_levels, text_dim):
        super().__init__()
        ...
        self.objectness_logits = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(in_channels, text_dim, kernel_size=1, stride=1),
        )
    ...
    def forward(self, features: List[torch.Tensor], category_embeddings: torch.Tensor):
        """
        Args:
        ...
        for level, x in enumerate(features):
            ...
            objectnesses = self.obj_scale * torch.einsum('bnh,bmh->bnm', F.normalize(objectness_feat), category_embeddings) + self.obj_bias
            pred_objectness_logits.append(objectnesses.permute(0, 2, 1).reshape(N, category_embeddings.shape[1], H, W).sigmoid())

        return pred_objectness_logits, pred_anchor_deltas


@HEADS.register_module()
class PromptAnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        super(PromptAnchorHead, self).__init__(init_cfg)
        ...
        self.main_head = StaRPNHead(self.in_channels, self.num_levels, text_dim)
        self.aux_head = StaRPNHead(self.in_channels, self.num_levels, text_dim)
    ...
    def forward_train(self,
        ...
        pred_logits_main, pred_anchor_deltas_main = self.main_head(head_features, category_embeddings)
        pred_logits_aux, pred_anchor_deltas_aux = self.aux_head(head_features, category_embeddings)
        main_results = dict(cls_scores_list=pred_logits_main,
                            bbox_preds_list=pred_anchor_deltas_main,)
        aux_results = dict(cls_scores_list=pred_logits_aux,
        ...
        with torch.no_grad():
            ...
            for img_id in range(len(img_metas)):
                singl_scores = main_loss_inputs[0][img_id].detach()
                num_classes = singl_scores.shape[-1]
                singl_bboxes = main_loss_inputs[1][img_id].detach()
                sinlge_score = singl_scores.flatten(0, 1)

                scores_per_img, topk_indices = sinlge_score.topk(
                    self.num_query, sorted=True)
                singl_bboxes = singl_bboxes[topk_indices // num_classes]
                proposals.append(singl_bboxes)
                proposals_scores.append(scores_per_img)
    ...
    def simple_test_rpn(self, x, category_embeddings, img_metas):
        ...
        pred_objectness_logits_main, pred_anchor_deltas_main = self.main_head(head_features, category_embeddings)
    ...
    def _get_topk(self, cls_score_list, bbox_pred_list, mlvl_priors, img_meta,
        ...
        for level_idx, (cls_score, bbox_pred, priors, stride) in \
            ...
            cls_score = cls_score.permute(1, 2, 0).flatten(0, 1)
