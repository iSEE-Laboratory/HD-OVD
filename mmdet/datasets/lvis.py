...
class LVISV05Dataset(CocoDataset):
    ...
    def evaluate(self,
        ...
        allowed_metrics = ['bbox', 'segm', 'proposal', 'proposal_fast', 'proposal_for_rcf']
        ...
        for metric in metrics:
            ...
            if metric not in result_files:
                if metric != 'proposal_for_rcf':
                    raise KeyError('{} is not in results'.format(metric))
            try:
                results_f = result_files['proposal'] if metric == 'proposal_for_rcf' else result_files[metric]
                lvis_dt = LVISResults(lvis_gt, results_f)
            except IndexError:
                print_log(
            ...
            iou_type = 'bbox' if metric in ['proposal', 'proposal_for_rcf'] else metric
            lvis_eval = LVISEval(lvis_gt, lvis_dt, iou_type)
            lvis_eval.params.imgIds = self.img_ids
            if metric == 'proposal':
                lvis_eval.params.use_cats = 0
                # lvis_eval.params.max_dets = list(proposal_nums)
                lvis_eval.evaluate()
                ...
                for k, v in lvis_eval.get_results().items():
                    if k.startswith('AR'):
                        ...
                        eval_results[k] = val
            elif metric == 'proposal_for_rcf':
                lvis_eval.evaluate_recall()
                lvis_eval.summarize_rcf()
                for k, v in lvis_eval.get_results().items():
                    if k.startswith('AR'):
                        val = float('{:.3f}'.format(float(v)))
                        eval_results[k] = val
