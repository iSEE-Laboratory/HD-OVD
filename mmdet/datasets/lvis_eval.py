...
class LVISEval:
    ...
    def evaluate_recall(self,):
        ...
        self.ious = [
            np.array(self.compute_iou(img_id, cat_id)).T
            for img_id in self.params.img_ids
            for cat_id in cat_ids
        ]

        self.params.use_cats = 1
        labels = []
        ignores = []
        for i, img_id in enumerate(self.params.img_ids):
            if len(self.ious[i]) > 0:
                temp_ignore = []
                for cat_id in self.params.cat_ids:
                    gt, dt = self._get_gt_dt(img_id, cat_id)
                    labels += [cat_id] * len(gt)
                    for g in gt:
                        temp_ignore += [1] if g["ignore"] else [0]
                temp_ignore = np.array(temp_ignore) == 1
                self.ious[i][temp_ignore, :] = -1
                ignores.append(~temp_ignore)
        labels = np.array(labels)
        ignores = np.concatenate(ignores)

        matched = _recalls(self.ious, [self.params.max_dets], self.params.iou_thrs)

        rare = np.ones((labels.shape[0])) == 0
        commom = np.ones((labels.shape[0])) == 0
        frequent = np.ones((labels.shape[0])) == 0
        for i in self.freq_groups[0]:
            rare = np.logical_or(rare, labels == i)
        for i in self.freq_groups[1]:
            commom = np.logical_or(commom, labels == i)
        for i in self.freq_groups[2]:
            frequent = np.logical_or(frequent, labels == i)
        recalls = np.zeros((matched.shape[0], matched.shape[1], 3))
        for i in range(matched.shape[0]):
            for j in range(matched.shape[1]):
                rare_idx = np.logical_and(ignores, rare)
                recalls[i, j, 0] = np.sum(matched[i, j][rare_idx]) / np.sum(rare_idx)
                commom_idx = np.logical_and(ignores, commom)
                recalls[i, j, 1] = np.sum(matched[i, j][commom_idx]) / np.sum(commom_idx)
                frequent_idx = np.logical_and(ignores, frequent)
                recalls[i, j, 2] = np.sum(matched[i, j][frequent_idx]) / np.sum(frequent_idx)
        self.recalls = np.mean(recalls, axis=0)
        self.all_recalls = np.mean(np.sum(matched, axis=-1) / np.sum(ignores))
    ...
    def summarize(self):
        ...
        if self.params.use_cats == 0:
            return
        for i in range(3):
            cat = ['r', 'c', 'f']
            key = "AR{}@{}".format(cat[i][0], max_dets)
            self.results[key] = self._summarize('ar', freq_group_idx=i)

    def summarize_rcf(self):
        """Compute and display summary metrics for evaluation results."""
        # if not self.eval:
        #     raise RuntimeError("Please run accumulate() first.")
        ...
        max_dets = self.params.max_dets
        key = "AR@{}".format(max_dets)
        self.results[key] = self.all_recalls
        for i in range(3):
            cat = ['r', 'c', 'f']
            key = "AR{}@{}".format(cat[i][0], max_dets)
            self.results[key] = self.recalls[0, i]
