...
class CrossEntropyLossCost:
    ...
    def __init__(self, weight=1., use_sigmoid=True, eps=1e-12, version='v1'):
    ...
    def _cross_entropy1(self, cls_pred, gt_labels):
        """
        Args:
            cls_pred (Tensor): Predicted classification logits, shape
                (num_query, num_class).
        ...
        # n = cls_pred.shape[1]
        # neg_cost = -(cls_pred[:, -1] + self.eps).log().reshape(-1, 1)
        # pos_cost = -(cls_pred[:, :-1] + self.eps).log()

        # cls_cost = (pos_cost[:, gt_labels] - neg_cost) / n
        # return cls_cost * self.weight
        cls_pred = cls_pred.softmax(-1)
        neg_cost = -(1 - cls_pred + self.eps).log() * (
            1 - self.alpha) * cls_pred.pow(self.gamma)
    ...
    def _cross_entropy2(self, cls_pred, gt_labels):
        """
        Args:
            cls_pred (Tensor): Predicted classification logits, shape
                (num_query, num_class).
    ...
    def __call__(self, cls_pred, gt_labels):
        ...
        if self.use_sigmoid:
            ...
            if self.version == 'v1':
                cls_cost = self._cross_entropy1(cls_pred, gt_labels)
            else:
                cls_cost = self._cross_entropy2(cls_pred, gt_labels)
