...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, attn_mask: torch.Tensor):

        xx = self.ln_1(torch.cat([x[1:], y]))
        y = y + self.attn(
            xx[[-1]],
            xx,
            xx,
...
class VisionTransformer(nn.Module):
    ...
    def interpolate_positional_embedding(self, size):
        if size == (self.grid, self.grid):
            return self.positional_embedding
        positional_embedding = self.positional_embedding[1:]
        positional_embedding = torch.reshape(positional_embedding, (self.grid, self.grid, -1)).permute(2, 0, 1).unsqueeze(0)
